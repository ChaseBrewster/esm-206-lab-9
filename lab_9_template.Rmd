...
mean_sqft <- homes_sub %>% 
  summarize(mean = mean(PricePerSqFt))
...
# Overall, heterosdcedasticity is OK, residuals normality definitely looks good, and conceptually and mathematically my model is making sense!
```
...
mid_aic <- AIC(homes_lm2)
final_aic <- AIC(homes_lm3)

# Answer: which would you pick? 

# Based on the AIC values, we would pick sat_aic. However, looking at this critically, we know the next two models are actually better choices. So, homes_lm3 is the best choice (just looking at the coefficient values, the heteroscedasticity, residual normality, and what makes the most sense realistically)
